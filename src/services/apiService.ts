...
export const getFarmScheduleEventsByFarmId = async (
  farmId: string
) => {
  const rawData = await fetch(`http://localhost:5228/api/Farms/GetAllFarmEvents/${farmId}`);
  const data = await rawData.json();
  return data;
};

export const getFarmerScheduleEventsByFarmerId = async (
  farmerId: string
) => {
  const rawData = await fetch(`http://localhost:5228/api/Farmers/GetFarmerEvents/${farmerId}`);
  const data = await rawData.json();
  return data;
};

export const getFeedDetailsByFarmerId = async () => {
  const rawData = await fetch(`http://localhost:5228/api/OrderFeed/GetOrdersSchedule/1`);
  const data = await rawData.json();
  return data;
};
...
export const getFeedEventsByFarmerId = async () => {
  const rawData = await fetch(`http://localhost:5228/api/OrderFeed/GetEvents/1`);
  const data = await rawData.json();
  return data;
};
